...
import type { ActionCardsSectionStoryblok } from "@/src/types/component-types-sb";
import { ActionCard } from "./action-card";

type ActionCardsSectionProps = {
  blok: ActionCardsSectionStoryblok;
};

export function ActionCardsSection({ blok }: ActionCardsSectionProps) {
    ...
    <section className="py-16 sm:py-20">
      <Container className="flex flex-col gap-10">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {cards.map((card) => (
            <ActionCard key={card._uid} blok={card} />
          ))}
        </div>
      </Container>
    </section>
